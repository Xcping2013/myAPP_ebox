...
u8 usmart_sys_cmd_exe(u8 *str)
{
	...
	{					   
		...
		case 1://帮助指令
			uart1.printf("\r\n");
#if USMART_USE_HELP
			uart1.printf("------------------------USMART V3.1------------------------ \r\n");
			uart1.printf("    USMART是由ALIENTEK开发的一个灵巧的串口调试互交组件,通过 \r\n");
			uart1.printf("它,你可以通过串口助手调用程序里面的任何函数,并执行.因此,你可\r\n");
			uart1.printf("以随意更改函数的输入参数(支持数字(10/16进制)、字符串、函数入\r\n");	  
			uart1.printf("口地址等作为参数),单个函数最多支持10个输入参数,并支持函数返 \r\n");
			uart1.printf("回值显示.新增参数显示进制设置功能,新增进制转换功能.\r\n");
			uart1.printf("技术支持:www.openedv.com\r\n");
			uart1.printf("USMART有7个系统命令:\r\n");
			uart1.printf("?:      获取帮助信息\r\n");
			uart1.printf("help:   获取帮助信息\r\n");
			uart1.printf("list:   可用的函数列表\r\n\n");
			uart1.printf("id:     可用函数的ID列表\r\n\n");
			uart1.printf("hex:    参数16进制显示,后跟空格+数字即执行进制转换\r\n\n");
			uart1.printf("dec:    参数10进制显示,后跟空格+数字即执行进制转换\r\n\n");
			uart1.printf("runtime:1,开启函数运行计时;0,关闭函数运行计时;\r\n\n");
			uart1.printf("请按照程序编写格式输入函数名及参数并以回车键结束.\r\n");    
			uart1.printf("--------------------------ALIENTEK------------------------- \r\n");
#else
			uart1.printf("指令失效\r\n");
#endif
			break;
		case 2://查询指令
			uart1.printf("\r\n");
			uart1.printf("-------------------------函数清单--------------------------- \r\n");
			for(i=0;i<usmart_dev.fnum;i++)uart1.printf("%s\r\n",usmart_dev.funs[i].name);
			uart1.printf("\r\n");
			break;	 
		case 3://查询ID
			uart1.printf("\r\n");
		...
		case 4://hex指令
			...
			if(i==0)//参数正常
			{
				...
				if(i==0)						  	//进制转换功能
				{
					uart1.printf("HEX:0X%X\r\n",res);	   	//转为16进制
				}else if(i!=4)return USMART_PARMERR;//参数错误.
				else 				   				//参数显示设定功能
				{
					uart1.printf("16进制参数显示!\r\n");
		...
		case 6://runtime指令,设置是否显示函数执行时间
			...
			if(i==0)//参数正常
			{
				...
				if(i==0)						   		//读取指定地址数据功能
				{
					if(USMART_ENTIMX_SCAN==0)uart1.printf("\r\nError! \r\nTo EN RunTime function,Please set USMART_ENTIMX_SCAN = 1 first!\r\n");//报错
					else
					{
						usmart_dev.runtimeflag=res;
						if(usmart_dev.runtimeflag)uart1.printf("Run Time Calculation ON\r\n");
						else uart1.printf("Run Time Calculation OFF\r\n"); 
					}
...
void usmart_exe(void)
{
	...
	for(i=0;i<pnum;i++)//输出参数
	{
		...
		{
			uart1.printf("%c",'"');			 
			uart1.printf("%s",usmart_dev.parm+usmart_get_parmpos(i));
			uart1.printf("%c",'"');
			temp[i]=(u32)&(usmart_dev.parm[usmart_get_parmpos(i)]);
		}else						  //参数是数字
		{
			temp[i]=*(u32*)(usmart_dev.parm+usmart_get_parmpos(i));
			if(usmart_dev.sptype==SP_TYPE_DEC)uart1.printf("%lu",temp[i]);//10进制参数显示
			else uart1.printf("0X%X",temp[i]);//16进制参数显示 	   
		}
		if(i!=pnum-1)uart1.printf(",");
	}
	uart1.printf(")");
	...
	if(rval==1)//需要返回值.
	{
		if(usmart_dev.sptype==SP_TYPE_DEC)uart1.printf("=%lu;\r\n",res);//输出执行结果(10进制参数显示)
		else uart1.printf("=0X%X;\r\n",res);//输出执行结果(16进制参数显示)	   
	}else uart1.printf(";\r\n");		//不需要返回值,直接输出结束
	if(usmart_dev.runtimeflag)	//需要显示函数执行时间
	{ 
		uart1.printf("Function Run Time:%d.%1dms\r\n",usmart_dev.runtime/10,usmart_dev.runtime%10);//打印函数执行时间 
	}	
}
//usmart扫描函数
